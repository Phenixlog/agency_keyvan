...
import { CHANNELS, PLAN_SCHEMA, fallbackPlan, gridRange, groupByDay, isChannel, isValidDay, missingSlots, monthGrid, parsePlan, ratioMismatch, resolveMonth, shiftMonth, toDay, weekGaps, PLAN_TILE_KINDS, PLAN_BACKGROUNDS, assembleCaption, CAPTION_CHANNEL_RULES } from "./model.ts";
...
test("parsePlan ne fait pas confiance au planneur", () => {
  ...
  assert.deepEqual(plan, [
    { day: "2026-09-26", channel: "instagram", angle: "Coulisses de l'atelier", creation: 1, idea: "", content: null },
    { day: "2026-09-29", channel: "instagram", angle: "b", creation: 0, idea: "Un bol fumant", content: { kind: "hook_photo", headline: "Un bol fumant", background: "brand" } },
  ]);
  assert.deepEqual(parsePlan(null, limits), []);
});

test("le plan est fait de tuiles : type, accroche, fond, en damier par canal, et rien n'est inventé par le code", () => {
  const limits = { days: ["2026-10-01", "2026-10-02", "2026-10-03", "2026-10-06"], taken: [], creations: 0 };
  const plan = parsePlan(
    {
      items: [
        { day: "2026-10-01", channel: "instagram", angle: "a", creation: 0, kind: "big_number", headline: ' « 6h » du matin ', background: "dark", idea: "" },
        { day: "2026-10-02", channel: "instagram", angle: "b", creation: 0, kind: "big_number", headline: "Encore un chiffre", background: "dark", idea: "" }, // même type, même fond → décalés
        { day: "2026-10-03", channel: "facebook", angle: "c", creation: 0, kind: "minitel", headline: "", background: "fluo", idea: "Une scène" }, // type et fond inconnus, sans accroche
        { day: "2026-10-06", channel: "instagram", angle: "d", creation: 0, kind: "quote", headline: "", background: "light", idea: "" }, // ni accroche ni photo → écartée
      ],
    },
    limits
  );
  assert.deepEqual(plan.map((p) => [p.day, p.content.kind, p.content.headline, p.content.background]), [
    ["2026-10-01", "big_number", "6h du matin", "dark"],
    ["2026-10-02", "list", "Encore un chiffre", "brand"],
    ["2026-10-03", "hook_photo", "Une scène", "brand"],
  ]);
});

test("les types et fonds du planificateur sont ceux des tuiles (parité)", async () => {
  const tiles = await import("../tiles/model.ts");
  assert.deepEqual([...PLAN_TILE_KINDS], Object.keys(tiles.TILE_KINDS));
  assert.deepEqual([...PLAN_BACKGROUNDS], [...tiles.BACKGROUNDS]);
});
...
test("fallbackPlan comble ce qui manque, mardi et jeudi d'abord, créations prêtes d'abord", () => {
  ...
  assert.deepEqual(plan[0].content, { kind: "big_number", headline: "Coulisses", background: "light" });
  assert.equal(plan[1].content, null);
...
test("le schéma du plan est strict", () => {
  ...
  assert.deepEqual(PLAN_SCHEMA.properties.items.items.required, ["day", "channel", "angle", "creation", "kind", "headline", "background", "idea"]);
  assert.deepEqual(PLAN_SCHEMA.properties.items.items.properties.channel.enum, Object.keys(CHANNELS));
});

test("assembleCaption : hashtags selon le canal, jamais dans le texte, jamais de doublon", () => {
  const out = assembleCaption("instagram", "Cuit ce matin, encore tiède.\n\n\nVenez le goûter #triche", ["#pasteldenata", "ajaccio", "pasteldenata", "boulangerie portugaise", "x", "y", "z"]);
  assert.equal(out, "Cuit ce matin, encore tiède.\n\nVenez le goûter\n\n#pasteldenata #ajaccio #boulangerieportugaise #x #y");
  assert.equal(assembleCaption("facebook", "Bonjour le quartier", ["a", "b"]), "Bonjour le quartier\n\n#a");
  assert.equal(assembleCaption("newsletter", "Objet\n\nCorps", ["a"]), "Objet\n\nCorps");
  assert.ok(Object.keys(CAPTION_CHANNEL_RULES).every((c) => c in CHANNELS) && Object.keys(CHANNELS).every((c) => c in CAPTION_CHANNEL_RULES), "une règle par canal");
});
