...
import { CARD_PAIRS, TILE_COPY_SCHEMA, TILE_KINDS, cardLines, cardPrompt, compareTexts, fallbackTilePlan, parseCardInfo, parseTilePlan, surfaceOf, tileCopyUserMessage, tilePrompt } from "./model.ts";
...
test("carte de visite : un nom et un contact au minimum, les coordonnées citées telles quelles au verso, jamais de mise en scène", () => {
  assert.equal(parseCardInfo({ name: "Julie De Nys" }), null, "sans contact, pas de carte");
  assert.equal(parseCardInfo({ phone: "+32 4 86 22 28 68" }), null, "sans nom, pas de carte");
  const info = parseCardInfo({ name: ' Julie "De Nys" ', role: "Architecte d’intérieur", phone: "+32 4 86 22 28 68", email: "contact@6avenueinterieur.com", website: "", address: "", social: "@6avenue.interieur", tagline: "Une adresse pour créer la vôtre" });
  assert.equal(info.name, "Julie De Nys");
  assert.deepEqual(cardLines("back", info), ["Julie De Nys", "Architecte d’intérieur", "+32 4 86 22 28 68", "contact@6avenueinterieur.com", "@6avenue.interieur"]);
  assert.deepEqual(cardLines("front", info), ["Une adresse pour créer la vôtre"]);
  const back = cardPrompt({ face: "back", info, background: "light", graphic: GRAPHIC, os: OS, hasLogo: true, brandName: "6avenue" });
  for (const expected of ['"Julie De Nys" (bold)', '"+32 4 86 22 28 68"', '"@6avenue.interieur"', "BACK of a business card", "no desk", "no photograph", "no QR code", "small, in a corner", "no other readable text"]) assert.ok(back.includes(expected), expected);
  assert.doesNotMatch(back, /Une adresse pour créer/);
  const front = cardPrompt({ face: "front", info, background: "brand", graphic: GRAPHIC, os: OS, hasLogo: true, brandName: "6avenue" });
  assert.match(front, /FRONT of a business card/);
  assert.match(front, /"Une adresse pour créer la vôtre"/);
  assert.match(front, /THE element of this face/);
  assert.doesNotMatch(front, /Julie/);
  assert.doesNotMatch(front, /\bmargins?\b/i);
  assert.equal(CARD_PAIRS.length, 3);
  for (const pair of CARD_PAIRS) assert.notEqual(pair.front, pair.back, "recto et verso ne partagent jamais le même fond");
});
