...
import { MAX_HISTORY_MESSAGES, MAX_MESSAGE_CHARS, SUGGESTIONS, buildExpertSystem, fallbackTitle, sanitizeHistory } from "./model.ts";
...
const CTX = {
  ...
  clientFeedback: [{ day: "2026-09-23", channel: "Instagram", comment: "Trop sombre, on ne voit pas le bol" }],
  decisions: [{ day: "2026-09-20", title: "Cadence 2x Instagram" }],
  today: "2026-09-21",
};

test("le prompt système contient tout ce que l'expert doit savoir de la marque", () => {
  const system = buildExpertSystem(CTX);
  for (const expected of ["« Atelier Lune »", "Ton : chaleureux", "Des objets qui durent", "- Toujours une trace", "Montrer l\u2019usage.", "1. 2026-09-21 · Social 1:1 · gardée · brief : Un bol fumant", "2026-09-25 · LinkedIn · Le bol du matin", "Date du jour : 2026-09-21", "Instagram · « Trop sombre", "2026-09-20 · Cadence 2x Instagram"]) {
...
test("le prompt système reste cohérent pour une marque encore vide", () => {
  const system = buildExpertSystem({ ...CTX, summary: "", canon: null, guidance: "", rules: [], creations: [], upcoming: [], clientFeedback: [], decisions: [] });
  for (const expected of ["(aucun résumé)", "(non structuré", "(aucune pour l’instant)", "(aucune)", "(aucune création pour l’instant)", "(rien de planifié)", "(aucun retour)"]) {
    assert.ok(system.includes(expected), expected);
  }
});
...
test("le titre de repli reprend les premiers mots, borné", () => {
  assert.equal(fallbackTitle("  Les   visuels sont trop sombres depuis la semaine dernière, non ?"), "Les visuels sont trop sombres depuis");
  assert.equal(fallbackTitle(""), "Conversation");
  assert.ok(fallbackTitle("Anticonstitutionnellement Anticonstitutionnellement Anticonstitutionnellement Anticonstitutionnellement").length <= 60);
});
