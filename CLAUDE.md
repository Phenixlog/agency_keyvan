...
- **Formats** (`lib/brand-os/model.ts`) : un format = ratio + résolution + **consigne de composition pour ce support** (où placer le sujet, où laisser de la place). Catalogue de 36 formats en 5 familles + `custom` (ratio libre parmi ceux de GPT Image + support décrit par l'utilisateur). Toujours passer par `resolveFormat(format, custom)` — jamais d'indexation directe de `IMAGE_FORMATS` avec une valeur venue d'un formulaire ou d'une création. Chaque création stocke `format_label` et `aspect_ratio` (nécessaires pour étiqueter et retoucher un format sur mesure). Les clés déjà stockées (`social_square`, `landscape`, `poster`, `print_a4`…) ne doivent jamais être renommées.
