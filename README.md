...
- `lib/tiles/` · tuiles avec texte (`model.ts` pur et testé : types de tuiles, surfaces écran / imprimé / signalétique (`surfaceOf`), rédaction, prompt assemblé par du code, contrôle du texte rendu ; `index.ts` : appels au modèle) ; servi par `POST /api/tile-copy`
